...
**50+ curated AI agent skills + a local semantic search engine powered by Gemma.**
...
|---|---|
| **`skills/`** | 50+ production-grade SKILL.md files for any IDE with agent support (Antigravity, Cursor, Windsurf, Claude Code) |
...
### AI & Agents
`ai-agents-architect` · `rag-engineer` · `prompt-engineer` · `langgraph` · `gemini-api-dev` · `embedding-strategies` · `mcp-builder` · `deep-research`
...
### Backend
`python-pro` · `fastapi-pro` · `typescript-pro` · `golang-pro` · `rust-pro` · `docker-expert` · `gcp-cloud-run` · `prisma-expert`

### Security
`security-auditor` · `find-bugs` · `systematic-debugging` · `codebase-audit-pre-push` · `verification-before-completion`

### DevOps
`docker-expert` · `gcp-cloud-run` · `github-actions-templates` · `git-pushing` · `git-advanced-workflows`
